...
def generate_launch_description():
    ...
    description_package = 'robo_arm_description'
    controller_package = 'robo_arm_controller'
    ...
    rsp = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [os.path.join(get_package_share_directory(description_package), 'launch/include' ,'rsp.launch.py')]), 
            launch_arguments={'HW_mode': HW_mode, 'cmd_mode': cmd_mode}.items()
    )

    controller_config = os.path.join(
        get_package_share_directory(controller_package), 'config', 'trajectory_controller.yaml'
    )
